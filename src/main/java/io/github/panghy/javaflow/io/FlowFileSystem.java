 ...
 * All operations return CompletableFuture objects that can be awaited within actors.
 * 
...
public interface FlowFileSystem {
   ...
   */
  CompletableFuture<FlowFile> open(Path path, OpenOptions... options);
   ...
   */
  CompletableFuture<Void> createDirectories(Path path);
   ...
   */
  CompletableFuture<List<Path>> list(Path directory);
   ...
   */
  CompletableFuture<Void> move(Path source, Path target);
