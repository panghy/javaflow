...
public interface RpcServiceInterface {
  ...
  /**
   * Registers this interface with the RPC transport using the specified endpoint ID.
   * This makes the interface's methods available for remote invocation.
   *
   * @param endpointId The endpoint ID to register with
   */
  default void registerRemote(EndpointId endpointId) {
    FlowRpcTransport.getInstance().registerEndpoint(endpointId, this);
  }
