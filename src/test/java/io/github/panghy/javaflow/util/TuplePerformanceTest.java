  ...
  public void run() {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      if (size != t.pack().length) {
        throw new RuntimeException(format("packed size did not match actual packed length: {0} --  {1} instead of {2}",
            t,
            tCopy.getPackedSize(),
            t.getPackedSize()));
      }
    ...
    System.out.printf("  Serialization rate:                    %f objects / \u03BCs%n", totalLength * 1000.0 /
                                                                                         packNanos);
